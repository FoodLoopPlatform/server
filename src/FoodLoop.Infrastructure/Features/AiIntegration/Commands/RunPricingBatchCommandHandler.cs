...
public class RunPricingBatchCommandHandler : IRequestHandler<RunPricingBatchCommand, Result<Unit>>
{
    ...
    public RunPricingBatchCommandHandler(
        ...
        ILogger<RunPricingBatchCommandHandler> _logger)
    {
        ...
        foreach (var storeGroup in storeGroups)
        {
            ...
            try
            {
                ...
                foreach (var decision in response.Decisions)
                {
                    ...
                    var candidate = candidates.FirstOrDefault(c => c.ProductId == productId);
                    if (candidate == null)
                    {
                        _logger.LogWarning("Candidate not found in current batch for ProductId '{ProductId}'. Skipping.", productId);
                        continue;
                    }
                    ...
                    if (org.AiOperatingMode == AiOperatingMode.Autonomous)
                    {
                        ...
                        var computedFloor = PriceFloorCalculator.Calculate(candidate.Product!.OriginalPrice, floorPolicy);
                        var proposedPrice = candidate.Product.OriginalPrice * (1.0m - (decimal)decision.DiscountPercentage / 100.0m);
                        ...
                        if (proposedPrice >= computedFloor)
                        {
                            var history = new PriceHistory
                            {
                                ...
                                OldOriginalPrice = candidate.Product.OriginalPrice,
                                OldDiscountedPrice = candidate.Product.DiscountedPrice,
                                NewOriginalPrice = candidate.Product.OriginalPrice,
                                NewDiscountedPrice = proposedPrice,
                                ChangeReason = "AI Autonomous Pricing",
                                ChangedBy = Guid.Empty
                            };
                            _dbContext.PriceHistories.Add(history);

                            candidate.Product.DiscountedPrice = proposedPrice;
                        ...
                        else
                        {
                            ...
                            _logger.LogWarning("Price floor violation for Product {ProductId}: proposed price {Proposed} is below calculated floor {Floor}.", productId, proposedPrice, computedFloor);
                        }
                    }
                    else
                    {
            ...
            {
                _logger.LogError(contractEx, "Contract validation failed for Store {StoreId} pricing batch. Skipping this store.", org.Id);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Exception while executing pricing batch for Store {StoreId}. Candidate updates aborted for this store.", org.Id);
            }
        }
